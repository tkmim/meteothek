...
class read:
    ...
    def simple_binary(path, nx, ny, nz, nt):
    ...
    # Records must be ordered as the GrADS binary data order.
        with open(path,'rb') as ifile:
            data=np.fromfile(path,dtype='float32',sep='').reshape(nt,nz,nx,ny,order='F')
    ...
    # "complex binary" means a binary file comprised of one element with x, y, z, t dimenstions.
    #
    ...
    def read_namelist_lambert(path, ft_grd):
        import f90nml
